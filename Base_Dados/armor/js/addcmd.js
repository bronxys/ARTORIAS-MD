...
const comandos = JSON.parse(fs.readFileSync("./Base_Dados/DADOS/comandos.json"))

const addComandosId = (from) => { 
const obj = { groupId : from, comandos:[]};
comandos.push(obj); 
fs.writeFileSync('./Base_Dados/DADOS/comandos.json', JSON.stringify(comandos))};
...
if (position !== false) { 
comandos[position].comandos.push(oq); fs.writeFileSync('./Base_Dados/DADOS/comandos.json', JSON.stringify(comandos))}
}
const deleteComandos = (from, oq) => { let position = false; Object.keys(comandos).forEach((i) => { if (comandos[i].groupId === from ){ position = i }}); if (position !== false) { comandos[position].comandos.splice(oq, 1); fs.writeFileSync('./Base_Dados/DADOS/comandos.json', JSON.stringify(comandos))}}
